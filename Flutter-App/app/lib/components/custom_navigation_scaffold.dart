...
/// [ListTile].
class CustomAdaptiveScaffoldDestination {
  final String title;
  final Icon icon;

  const CustomAdaptiveScaffoldDestination({
    required this.title,
    required this.icon,
  });
}
...
class CustomAdaptiveNavigationScaffold extends StatelessWidget {
  const CustomAdaptiveNavigationScaffold({
    Key? key,
    this.appBar,
    required this.body,
  ...
  /// values.
  final List<CustomAdaptiveScaffoldDestination> destinations;
  ...
  Drawer _defaultDrawer(List<CustomAdaptiveScaffoldDestination> destinations) {
    return Drawer(
      child: ListView(
        children: [
  ...
  Widget _buildBottomNavigationScaffold() {
    ...
    final drawerDestinations = destinations.length > bottomNavigationOverflow
        ...
        : <CustomAdaptiveScaffoldDestination>[];
    return Scaffold(
      key: key,
      body: body,
      appBar: appBar,
            ...
            for (final destination in destinations)
              Card(
                  color: destinations.indexOf(destination) == selectedIndex
                      ? ThemeHelper.theme.primaryColorDark.withOpacity(.1)
  ...
  void _destinationTapped(CustomAdaptiveScaffoldDestination destination) {
