...
void FA22RF::decode(bool state, unsigned long value) {
    if (state) {
...
void FA22RF::performAlarm(const char *rawCode) {
...
void FA22RF::sendShortPulse() const {
    digitalWrite(Config::TRANSMITTER_433_PIN, HIGH);
...
void FA22RF::sendStartPulse() const {
    digitalWrite(Config::TRANSMITTER_433_PIN, HIGH);
    delayMicroseconds(shortGapOn);
    digitalWrite(Config::TRANSMITTER_433_PIN, LOW);
    delayMicroseconds(shortGapOff);
}
